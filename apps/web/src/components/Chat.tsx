...
function IngestDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setResult(`OK: ${r.chunkCount} chunks indexados com sucesso no Vectorize 1024 + R2.`);
      setTitle("");
      setText("");
    } catch (e) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg border-border/60 shadow-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Cole o texto ou documento (PDF/Markdown/TXT). O backend divide em chunks (~2800 chars), gera embeddings de 1024 dimensões com Perplexity Embed e grava no Vectorize + R2.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button onClick={submit} disabled={busy || !title.trim() || !text.trim()} className="gap-2">
            {busy ? (
              <>
                <Spinner /> Indexando no Vectorize…
              </>
            ) : (
              <>
                <UploadSimple className="size-4" /> Indexar documento
              </>
            )}
          </Button>
        </DialogFooter>
