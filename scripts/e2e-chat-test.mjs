...
// Requer: api rodando (pnpm dev ou wrangler dev) + .env com JWT_SECRET.
//
// Fluxo:
//   1. health check
...
async function readSSE(path, { token, body }) {
  ...
  if (!res.ok || !res.body) throw new Error(`chat HTTP ${res.status}: ${(await res.text()).slice(0, 200)}`);
...
async function main() {
  ...
  const secret = process.env.JWT_SECRET ?? loadEnvVar("JWT_SECRET");
  if (!secret) throw new Error("JWT_SECRET não achado no env nem no .env");
  const devTok = await req("/api/v1/auth/dev-token", { method: "POST", body: { setupSecret: secret } });
  if (!devTok.ok || !devTok.json?.token) throw new Error(`dev-token falhou: HTTP ${devTok.status} ${devTok.text.slice(0, 120)}`);
  const token = devTok.json.token;
  ...
  // ---- TESTE A: ephemeral (nunca persiste) ----
  {
    ...
    if (sse.conversationId !== cid) throw new Error(`[e2e:A] conversationId divergiu (${sse.conversationId} ≠ ${cid})`);
    console.log(`[e2e:A] done ok (tokens=${sse.tokens}, fullText=${(sse.done.fullText ?? "").length} chars)`);
    const ids = await listIds();
    if (ids.includes(cid)) throw new Error("[e2e:A] RASTRO! conversa ephemeral apareceu no histórico");
  ...
  // ---- TESTE B: persiste → verifica → APAGA (delete em finally) ----
  {
    ...
    try {
      ...
      const ids = await listIds();
      if (!ids.includes(cid)) throw new Error("[e2e:B] conversa persistida NÃO apareceu no histórico");
      console.log("[e2e:B] persistiu ✓ — apagando…");
    } finally {
      const del = await req(`/api/v1/conversations/${cid}`, { method: "DELETE", token });
      if (!del.ok) console.error(`[e2e:B] WARN delete falhou: HTTP ${del.status} — rode: node scripts/cleanup-conversations.mjs`);
      else console.log("[e2e:B] DELETE ok");
      const ids = await listIds();
      if (ids.includes(cid)) throw new Error("[e2e:B] RASTRO! conversa continua no histórico após DELETE");
      console.log("[e2e:B] apagada e verificada ✓");
    }
  }
